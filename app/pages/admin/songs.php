		...
		if($_SERVER['REQUEST_METHOD'] == "POST")
		{
			...
			if(!empty($_FILES['file']['name']))
			{
				...
				if($_FILES['file']['error'] == 0 && in_array($_FILES['file']['type'], $allowed))
				{
					...
					$destination_audio = $folder. $_FILES['file']['name'];

					move_uploaded_file($_FILES['file']['tmp_name'], $destination_audio);
			...
			if(empty($errors))
			{
				...
				$query = "insert into songs (title,image,file,user_id,category_id,artist_id,date,views,slug,likes,featured) values (:title,:image,:file,:user_id,:category_id,:artist_id,:date,:views,:slug,:likes,:featured)";
